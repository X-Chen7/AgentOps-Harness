...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_skills.add_argument("--check", dest="check_only", action="store_true", help="Compare files without copying")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "check":
            return check_mod.cmd_check(root, strict=args.strict, backend=args.backend, compile=args.compile)
        ...
        if args.command == "install-hooks":
            return hooks_mod.install_hooks(root, force=args.force)
