...
- 变更与 Git 协作：`.harness/changes/`、`script/sync-changes.ps1`、`script/harness-git.ps1`
- 多智能体流水线：`.harness/pipelines/desktop-pipeline.json`、`.harness/agents/pipeline/`
- 模板与基准：`.harness/templates/`、`benchmark/`
- 初始化：`harness-template/`、`script/harness-init.ps1`
...
F-009 是一个端到端演示 feature：为 `script/harness-check.ps1` 增加流水线终态一致性校验，并完整跑通 `analysis -> coding -> test -> review -> Git/PR`。
